...
// MIT license, edited by Arduino245 to work with some more modules
...
static const uint16_t ILI932x_regValues[] PROGMEM = {
  ...
  ILI932X_DRIV_WAV_CTRL    , 0x0700, // If image is mirrored, change to 0x0100
...
// Spend definitely too much time here...
...
uint16_t Adafruit_TFTLCD::readPixel(int16_t x, int16_t y) {
  ...
  if((driver == ID_932X) || (driver ==ID_S6D0154) || (driver == ID_ILI9327) || (driver == ID_ILI9488)) {
    ...
    return (((uint8_t)r & B11111000) << 8) |
	  ...
	  //CD_COMMAND; write8(0x2A); CD_DATA; write8(x); write8(0x00); write8(x); write8(0x00); 
	  //CD_COMMAND; write8(0x2B); CD_DATA; write8(y); write8(0x00); write8(y); write8(0x00);
	  //writeRegister8(0x2A, x);
	  //writeRegister8(0x2B, y);
